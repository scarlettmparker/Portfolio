...
export const fragmentShader = `
    ...
    void main() {
        ...
        float noise = snoise(vec3(vUv * 3.0, time * 0.5));
        vec4 overlayColor = vec4(1.0, 0.2, 0.8, 0.7) * (noise + 0.5);
        vec4 finalColor = mix(baseColor, overlayColor, overlayColor.a);

        // apply glow effect by boosting brightness
        finalColor.rgb += overlayColor.rgb * 0.5;
        finalColor.a = baseColor.a * 0.7;

        // pulsating pink overlay
        float pulsate = 0.5 + 0.5 * sin(time * 2.0);
        vec4 pulsatingOverlay = vec4(1.0, 0.2, 0.8, 0.3) * pulsate;

        finalColor.rgb += pulsatingOverlay.rgb * 0.5;
